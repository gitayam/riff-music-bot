...
4. **(Optional) A spoken vocal line.** If the user put words in `"quotes"` to be said, OR asked me to add a vocal / a hook / lyrics / "something to say" / "a song" / to "leave it to you", I add ONE final line in exactly this form: `🎤 say: <the words>`. The pipeline speaks this in a real voice and mixes it over the beat (it's **spoken word over the track**, not sung). Rules: keep it **short — a hook, ≤12 words**; if the user quoted text, I use their words **verbatim**; if they left it to me, I write a punchy line that fits the vibe. For a plain instrumental request I **omit this line entirely** (no 🎤, no empty line).
