...
- **P3 — Steering + seeds. ✅ Steerable from the browser (Jun 22).** `radio-compose.mjs` takes a steer
  ...
  **`POST /steer`** → writes that file, so the **player has darker/brighter · faster/slower · denser/sparser
  toggle buttons** that nudge the live stream from the next segment. Deterministic + tested. **Next:** a
  Discord command to steer + seed from time-of-day / channel mood (Situation D) / trending. **Stretch:**
  off-Cloudflare relay into a Discord voice channel.
