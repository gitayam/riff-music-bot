...
- **P1 — Containerized render. ◐ Render microservice + image BUILT (`container/`, Jun 22); CF wiring is deploy-time.**
  ...
  both documented in `container/README.md` (direct-binding vs the Queues→R2 scalable target). **Next:** build
  the image on a Docker host; then wire Worker → Queues → Container → R2 so `/render` (and the Discord
  follow-up) fill `audio_url`. *The render service itself is proven; lifting it onto CF is the remaining step.*
