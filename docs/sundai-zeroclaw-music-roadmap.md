...
- **P3 — Discord-native. ✅ Interactions webhook BUILT (`worker/`, Jun 22; Tier-A).** `POST /discord/interactions`
  ...
  **verifies Discord's Ed25519 signature** (WebCrypto, app public key `DISCORD_PUBLIC_KEY`), answers the PING
  ...
  link. Registering the slash command + setting the Interactions Endpoint URL are one-time operator steps.
  *The Discord path now runs on Cloudflare — the daemon + watcher are no longer required for it.*
