...
### Tier A — Strudel-native arrangement (fast path; one render) — ✅ VERIFIED WORKING
> A 4-section `arrange([4,intro],[8,verse],[8,chorus],[4,outro])` rendered the **full 48 s** (24 cycles, auto-sized by `strudel-cycles.sh`) — not cut to the intro, non-silent with dynamics. So the render pipeline already supports songs; the remaining work is the **agent composing** them (a soul directive for "make a full song"). Inline sections in `arrange()` are most robust; `const` sections also evaluate.
