...
- **P3 — Steering + seeds. ✅ Local file-based steering built (Jun 22).** `radio-compose.mjs` takes a
  steer hint (`RADIO_STEER`, e.g. `darker faster`) that biases mode/tempo/density; `radio.sh` re-reads
  `<outdir>/steer` each segment, so `echo 'darker faster' > <outdir>/steer` nudges the live stream from
  the next segment on. Deterministic + tested. **Next:** a `POST /steer` endpoint (the `--serve`
  `python -m http.server` is static — needs a tiny request handler) + a Discord command to write the
  steer; seed from time-of-day / channel mood (Situation D) / trending. **Stretch:** off-Cloudflare
  relay into a Discord voice channel.
