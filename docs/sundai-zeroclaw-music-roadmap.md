...
- **P0 — Worker shell. ✅ BUILT (`worker/`, Jun 22).** Ports `/generate` + `/render` (the `api-server.py`
  surface) to a Cloudflare Worker that calls OpenAI (`gpt-5.4`) via `fetch` and returns code + `share_url`
  — **Tier-A link only, no render** (`audio_url` is always `null`; render is P1). `POST` is bearer-gated
  (`MUSIC_API_TOKEN`), `/generate` auto-repairs invalid Strudel by re-prompting with the gate error
  (mirrors `generate_valid`), and the `share_url` base64 is **byte-identical to the local Python/Node
  systems** (unit-tested) so edge links play exactly like laptop links. The OpenAI call follows the gpt-5.4
  rules (flat `reasoning_effort`, `max_completion_tokens`, no `temperature`); `OPENAI_BASE_URL` can point at
  an **AI Gateway**. **Tested:** `worker/test.sh` → 12 pure-helper unit tests + a live `wrangler dev` pass
  (real `workerd`) against a **mock OpenAI** covering health/help/CORS/auth(401)/validation(400,422) and the
  full `/generate`→200 code+link path — deterministic, offline, no key, no cost; wired into `scripts/test.sh`
  (now **10 suites**). `wrangler deploy --dry-run` bundles clean (8.6 KiB). **Next:** set the two secrets +
  a custom-domain route to retire the quick tunnel. *Proves the orchestrator runs on the edge.*
- **P1 — Containerized render. ◐ Render microservice + image BUILT (`container/`, Jun 22); CF wiring is deploy-time.**
  Built the unit that runs inside the Container: **`container/server.mjs`**, a dep-free HTTP service wrapping
  the proven faithful engine — `POST /render {code,cycles,format}` → headless-Chromium render → ffmpeg →
  real audio bytes (`audio/*`), `422` if it won't render, with a timeout + one retry. This is the correct
  edge decomposition: the **Worker** does orchestration + the LLM (Strudel code), the **Container** does the
  binary-dependent render the Worker can't. Plus **`container/Dockerfile`** (linux/amd64, Playwright base +
  ffmpeg + the engine + cached sample packs). **Tested end-to-end locally as a node process** (`container/test.sh`,
  wired into `scripts/test.sh`): renders a real 909 pattern → 40 KB mp3 and 705 KB WAV (RIFF-verified),
  rejects `[…]`-wrap + prose → 422, missing code → 400. ⚠ The **Docker image build is unverified this session**
  (no Docker daemon) and the **CF Container/Queues/R2 wiring is deploy-time** (needs a Workers Paid account) —
  both documented in `container/README.md` (direct-binding vs the Queues→R2 scalable target). *The render
  service itself is proven; lifting it onto CF is the remaining step.*
- **P1 last mile — Worker renders audio via the service → R2 → served. ✅ BUILT + wired (`worker/`, Jun 22).**
  Connected the two big pieces: the Worker now calls the render service (`RENDER_SERVICE_URL`), stores the
  audio in **R2** (`AUDIO` bucket), and returns a served **`audio_url`** (`GET /audio/<key>`). `/render`
  renders by default; `/generate`/`/modify` render on `render:true` (so plain generate stays fast Tier-A).
  Rendering is **best-effort** — on failure the response degrades to `audio_url:null` + a `render_error`
  field (you always get the code + play link). The `tracks` table + history now carry `audio_url`.
  **Tested end-to-end in real `workerd`** (`wrangler dev` + **local R2** + a mock render service + mock
  OpenAI): `/render` → `audio_url`, `GET audio_url` → the stored bytes from R2, `/generate render:true` →
  `audio_url`, plain `/generate` → null (Tier-A), and a render-failure → graceful degrade. `worker/test.sh`
  32 unit + the audio-flow checks; `scripts/test.sh` **11/11**; dry-run bundles DO+D1+R2+cron clean (25.9 KiB).
  Reviewed via vibe-orchestrator. **The only un-done part is deploying the render service as a CF Container**
  (Docker host + Workers Paid account) — the edge orchestration around it is complete and proven.
- **P2 — State + persistence. ◐ Modify chain + D1 history BUILT (`worker/`, Jun 22); only Vectorize remains.**
  Per-session **Durable Object** (`src/session.js`, SQLite-backed) holds the ordered version chain, and
  **`POST /modify {session_id, instruction}`** loads the latest version, has `gpt-5.4` edit the code (same
  validate + auto-repair gate), stores the new version, and returns the new code + `share_url` + a **code
  `diff`** + `version`/`parent_id` — Situation E, **the demo's core differentiator, on the edge.** Plus
  **cross-session history in D1** (Contract 5): every generate/modify/render is logged as a `tracks` row
  (`src/store.js`; schema `migrations/0001_create_tracks.sql`, also lazily created), queryable via
  **`GET /history?session_id=…`** (bearer-gated, newest-first); persistence is best-effort (a D1 hiccup
  never breaks the music response); and a **daily Cron Trigger** (`scheduled()` → `pruneTracks`) prunes rows
  older than `RETENTION_DAYS`=30 so the log can't hit D1's 10 GB cap (retention shipped in the same change,
  per the Cloudflare lessons). **Tested end-to-end in real `workerd`** (`wrangler dev` + mock OpenAI,
  `--persist-to` isolated → hermetic): generate→v1, modify→v2 (parent 1, diff), modify→v3 chains on v2,
  unknown session→404; `/history` scoped + global + 401; and a full **retention E2E** — backdate a row via
  `d1 execute`, fire `/__scheduled`, old row pruned + recent kept. **Still P2:** **embeddings in Vectorize**
  ("more like this") — deferred because Vectorize has no real local-dev emulation (needs an account to
  verify), unlike D1/DO. *The modify loop + version history + durable cross-session history now run on the edge.*
  **➕ Jun 22 — "more like this" BUILT (D1-cosine; Vectorize = scale path).** `POST /similar {text|track_id}`
  embeds the query (OpenAI `text-embedding-3-small`) and ranks stored tracks by **cosine similarity** in the
  Worker (embeddings stored in a `tracks.embedding` column, populated best-effort + **opt-in** via
  `EMBED_TRACKS`). Implemented with a D1-backed scan rather than Vectorize precisely *because* it's then
  fully local-testable (and correct at this project's scale); the roadmap's Vectorize is the unbounded-scale
  swap (same `/similar` contract). **Tested in real `workerd`** (mock embeddings = deterministic
  bag-of-keywords → predictable ranking): `text` query ranks the matching track first, `track_id` excludes
  itself, 404/400 paths, and `/history` no longer leaks the embedding blob. So **P2 is now fully built** on
  the edge (D1 history + modify chain + similarity); Vectorize remains only as the scale-up option.
- **P3 — Discord-native. ✅ Interactions webhook BUILT (`worker/`, Jun 22; Tier-A).** `POST /discord/interactions`
  is the event-driven replacement for the REST-poll `strudel-watch.py` watcher — no laptop. The Worker
  **verifies Discord's Ed25519 signature** (WebCrypto, app public key `DISCORD_PUBLIC_KEY`), answers the PING
  handshake (PONG), and for a slash command **acks within 3 s with a deferred response (type 5)** then
  composes the Strudel in `waitUntil()` and **edits the @original message** (via the interaction token — no
  bot token needed) with the code + `strudel.cc` link, persisting a `discord`-source track to D1. Auth here
  is the signature, not the bearer. **Tested end-to-end in real `workerd`** (`wrangler dev`; a generated
  Ed25519 keypair signs the requests, a mock Discord API records the follow-up): signed PING→PONG, bad
  signature→401, missing headers→401, slash command→deferred ack, and the async follow-up posts the play
  link. *The Discord path now runs on Cloudflare — the daemon + watcher are no longer required for it.*
  **➕ Jun 22 — registration tool shipped:** the webhook is inert until the `/riff` command is registered,
  so `worker/register-command.mjs` does the one-time `PUT /applications/{id}/commands` (guild-scoped =
  instant, or global). Tested (injected-fetch unit tests + a real-process run against the mock Discord).
  **➕ Jun 22 — audio attached:** with P1's render path wired, the Discord follow-up now **renders the audio
  and attaches the mp3** (multipart upload via the interaction token — plays inline in Discord), degrading
  to code + link if render fails/unconfigured. So a Discord slash command → **real audio in the channel,
  fully on Cloudflare.** Tested in real `workerd` (mock render + mock Discord recording the multipart): the
  follow-up carries `riff.mp3` + the link, and a render-failure degrades to text-only.
...
- **P3 — Steering + seeds. ✅ Steerable from the browser (Jun 22).** `radio-compose.mjs` takes a steer
  ...
  **`POST /steer`** → writes that file, so the **player has darker/brighter · faster/slower · denser/sparser
  ...
  overrides. **Channel-mood ✅ (Jun 22 — Situation D):** `scripts/radio-mood.py` polls a channel's
  recent messages, classifies the vibe (dark↔bright × calm↔hype, via a pure keyword/emoji/shouting
  heuristic) into a steer hint, and writes the same steer file — so the stream **matches the vibe of
  the channel right now** ("match #general"). Opt-in (`RADIO_MOOD_CHANNEL` + `RADIO_STEER_FILE`); REST-only,
  mirrors the watcher. Verified end-to-end: sad/tired chatter → `dark` → minor mode + lower bpm; hype
  (🔥, ALL-CAPS, !!!) → `fast dense`; a faded mood clears the steer (reverts to the time-seed). Tested
  via `test-radio-mood.py` (17 cases, pure classifier + injected-fetch sample→write). **Remaining:** seed
  from *trending*. **Stretch:** off-Cloudflare relay into a Discord voice channel.
...
- **Jun 22 — Make the edge stack actually deployable: `/riff` command-registration tool + a turnkey DEPLOY runbook.** With the non-account-gated *features* built, the honest highest-value move was closing the gap between "tested components" and "a deployable, usable system" — not inventing a marginal feature. The concrete gap: the **P3 Discord webhook is inert until its slash command is registered**, and there was no tool for it. Built **`worker/register-command.mjs`** — the one-time `PUT /applications/{id}/commands` for `/riff` (a required `prompt` string option), guild-scoped (instant) or global; idempotent. `registerCommands(env, fetch)` takes an injected fetch so it's **fully unit-testable** (6 cases: command shape, guild-vs-global URL, the PUT body/auth-header, missing-env→exit 2, non-OK→exit 1) plus a real-process integration run against the mock Discord in `worker/test.sh`. And **`worker/DEPLOY.md`** — the ordered, copy-paste go-live runbook (create D1/R2 → secrets → deploy → wire the Interactions URL + register the command → build the render Container → verify), with rollback. `worker/test.sh` 45 unit + the register integration; `scripts/test.sh` **12/12**. Reviewed via vibe-orchestrator. This makes the user's actual next step (provision a Workers Paid account + Docker host, then deploy) a single checklist — the remaining blocker is purely the account/host, not code.
- **Jun 22 — Radio reacts to the channel's mood (Situation D, Phase-4 P3 "Next").** With Phase 3 feature-complete on the edge and the remaining big items account-gated (deploy the render Container, lift the radio to CF), picked the highest-impact *fully-verifiable, non-gated* roadmap item: the radio's last reactive case — "match the vibe of #general right now." Built `scripts/radio-mood.py`: it polls a channel's recent messages, classifies the vibe with a **pure keyword/emoji/shouting heuristic** (dark↔bright × calm↔hype) into a steer hint, and writes the **same steer file** the player buttons / chat `!radio` / `radio.sh` already use — so the live stream drifts to match the community. Opt-in (`RADIO_MOOD_CHANNEL` + `RADIO_STEER_FILE`; dormant otherwise), REST-only, mirroring the watcher. Chosen over the native Discord voice-message bubble (whose Discord-acceptance via interaction webhooks I can't verify in-session — a gamble) precisely because this is **end-to-end verifiable locally**: the classifier is pure/deterministic, and I confirmed the chain — sad/tired chatter → `dark` → minor mode + lower bpm; hype (🔥/ALL-CAPS/!!!) → `fast dense` → major + higher bpm; a faded mood clears the steer (reverts to the time-seed). **Tested:** new `scripts/test-radio-mood.py` (17 cases: mood axis, energy axis, emoji, neutral→'', malformed-message safety, and the sample→steer-file cycle with an injected fetch); added to `scripts/test.sh` (**12/12 suites**). Reviewed via vibe-orchestrator. **Remaining radio seed:** *trending*. The non-account-gated roadmap is now essentially exhausted — what's left is the CF deploy (render Container + radio lift), which needs a Workers Paid account + a Docker host.
- **Jun 22 — "More like this" (P2's last feature): similarity search via embeddings.** With Phase 3 otherwise complete end-to-end (only deploy-gated bits left), built the last unbuilt P2 feature — the "remix the vibe of that track" capability. `POST /similar {text}` or `{track_id}` embeds the query (OpenAI `text-embedding-3-small`) and ranks stored tracks by **cosine similarity**, returning the top matches. Chosen over deploying the Container (needs a Docker host + paid account, not in-session). Implemented with **embeddings stored in a D1 `tracks.embedding` column + an in-Worker cosine scan** rather than Vectorize — *deliberately*, because that's fully local-testable and correct at this project's scale; the roadmap's **Vectorize is documented as the unbounded-scale swap** (same `/similar` contract), exactly like the Container is the scale path for render. Embedding-on-write is **best-effort + opt-in** (`EMBED_TRACKS`, off by default → no per-write API cost, feature dormant). `/history` now strips the bulky embedding blob. **Tested in real `workerd`** (mock embeddings return a deterministic bag-of-keywords vector → predictable ranking): a `text` query ranks the matching dark/minor track first, `track_id` similarity excludes itself + returns others, 404/400 validation, and the embedding-leak guard. `worker/test.sh` 39 unit + the similarity integration checks; `scripts/test.sh` **11/11**; dry-run bundles clean (31.8 KiB). Reviewed via vibe-orchestrator. **Phase 3 is now feature-complete on the edge** (generate/modify/render-audio/Discord/history/similarity); the only remaining work is deploying the render service as a CF Container (Docker host + paid account).
- **Jun 22 — Real audio in Discord, on the edge (P3 + P1 integration): the slash-command follow-up now attaches the rendered mp3.** With the render path wired (previous iteration), the obvious next move was to make "make it sound real" land in the actual chat surface, not just the HTTP API. Chosen over Vectorize (a stretch with no real local emulation) and over deploying the Container (needs a Docker host + paid account — not in-session). Now `POST /discord/interactions` → deferred ack → in `waitUntil()` it composes the Strudel, **renders the audio via the render service, and PATCHes the @original message as a multipart upload with `riff.mp3` attached** (plays inline in Discord), still carrying the `strudel.cc` link. Best-effort — if render fails/unconfigured it degrades to the code + link text. Refactored `tryRender` to share a `renderBytes` helper with the Discord path. **Tested end-to-end in real `workerd`** (mock render service returns audio; mock Discord API records the multipart PATCH; a `FAILME` prompt forces an unrenderable code to exercise the degrade): the follow-up carries `riff.mp3` + the link, and the render-failure path is text-only. `worker/test.sh` 32 unit + the Discord-audio + degrade checks; `scripts/test.sh` **11/11**; dry-run bundles clean (27.3 KiB). Reviewed via vibe-orchestrator. **The edge stack is now complete end-to-end including audio in Discord;** the only un-done part is deploying the render service as a CF Container (Docker host + paid account). Optional remaining: Vectorize "more like this", and the native Discord voice-message bubble (waveform/Opus) as polish.
- **Jun 22 — Phase 3 P1 (last mile): the edge Worker now produces + serves REAL AUDIO (render service → R2 → /audio).** The Worker and the render service both existed but weren't connected — the Worker still returned `audio_url:null`. Wired them: the Worker calls the render service (`RENDER_SERVICE_URL`), stores the audio in **R2** (`AUDIO` bucket), and returns a served **`audio_url`** at `GET /audio/<key>`. `/render` renders by default; `/generate`/`/modify` render on `render:true` (plain generate stays fast Tier-A). Rendering is **best-effort** — a failure degrades to `audio_url:null` + a `render_error` field, so you always get the code + play link (same philosophy as the D1 best-effort persistence). The `tracks` table + `/history` now carry `audio_url`. Chosen over Vectorize (a stretch item with no real local emulation) because this connects the two largest pieces I'd built into a working whole, and it's **fully locally testable** — `wrangler dev` provides a **local R2**, and the render service runs as a plain node process (the real engine is already covered by `container/test.sh`, so the Worker test mocks it for speed/determinism). **Tested end-to-end in real `workerd`**: `/render` → `audio_url`; `GET audio_url` → the stored bytes from R2; `/generate render:true` → `audio_url`; plain `/generate` → null; render-failure → graceful degrade. `worker/test.sh` 32 unit + the audio-flow integration checks; `scripts/test.sh` **11/11**; dry-run bundles DO+D1+R2+cron clean (25.9 KiB). Reviewed via vibe-orchestrator. **The whole edge stack now works end-to-end** (prompt → code → modify → history → Discord → rendered audio); the **only remaining step is deploying the render service as a CF Container** (Docker host + Workers Paid account). Plus Vectorize "more like this" (optional stretch).
- **Jun 22 — Phase 3 P1 (render service): the containerized render microservice — the off-laptop audio crux.** Stopped deferring P1 (deferred 3×) and tackled its verifiable heart. The crux per the roadmap is "render + transcode must run in a Container." Built **`container/server.mjs`** — the unit that runs inside the CF Container: a dep-free HTTP service wrapping the *proven* faithful engine (`POST /render {code,cycles,format}` → headless-Chromium render → ffmpeg → real audio bytes; 422 if it won't render; timeout + one retry). This is the right edge decomposition — the **Worker** does orchestration + the LLM, the **Container** does the binary-dependent render — and it's the render half of `api-server.py` extracted with a clean HTTP contract for the Worker to call. Plus **`container/Dockerfile`** (linux/amd64, Playwright base + ffmpeg + engine + cached packs). **Tested end-to-end locally as a node process** (the engine works on this Mac): renders a 909 pattern → 40 KB mp3 + 705 KB WAV (RIFF-verified), rejects `[…]`-wrap + prose → 422, missing code → 400, 404; wired into `scripts/test.sh` (**11/11 suites**). **Honestly scoped:** the Docker image build is *unverified this session* (Docker daemon down) and the **CF Container/Queues/R2 wiring is deploy-time** (needs a Workers Paid account) — both documented in `container/README.md` (direct binding vs the Queues→R2 scalable target that fills `audio_url` and upgrades the Discord follow-up to attached audio). Reviewed via vibe-orchestrator. **Remaining Phase 3:** build the image on a Docker host + wire it to the Worker; Vectorize "more like this". The render service is proven; lifting it onto CF is the last step.
- **Jun 22 — Phase 3 P3: Discord-native Interactions webhook on the Worker — the laptop is no longer required for Discord.** With the edge Worker (P0), modify chain (P2 DO), and history (P2 D1) shipped, the highest-impact remaining move for the "off localhost" thesis was getting **Discord itself** off the laptop (the daemon + REST-poll watcher are the last laptop-bound pieces). Chosen over P1 (container audio — needs Docker + a paid account, can't verify in-session) and Vectorize (poor local emulation) because the crux — **Ed25519 signature verification** — is fully locally testable. Built `src/discord.js` + `POST /discord/interactions`: the Worker verifies Discord's signature (WebCrypto Ed25519, app public key), PONGs the PING handshake, and **deferred-acks a slash command within 3 s** then composes the Strudel in `waitUntil()` and **edits the @original message** (authorized by the interaction token — no bot token) with the code + `strudel.cc` link, logging a `discord` track to D1. Auth is the signature (not the bearer). **Tested end-to-end in real `workerd`** (a generated Ed25519 keypair signs the requests; a mock Discord API records the follow-up; mock OpenAI composes): signed PING→PONG, bad-sig→401, missing-headers→401, slash command→deferred ack (type 5), and the async follow-up posts the play link. `worker/test.sh` now 30 unit + 33 integration checks; `scripts/test.sh` **10/10**; `wrangler deploy --dry-run` bundles clean (22.7 KiB). Reviewed via vibe-orchestrator. **Tier-A:** posts code + link today; the rendered audio attaches when P1 (container render) lands. **Remaining Phase 3:** P1 container render (real `audio_url`) + Vectorize "more like this".
- **Jun 22 — Phase 3 P2 (history slice): cross-session `tracks` history + retention cron in D1.** With P0 (edge Worker) and the modify chain (DO) shipped, the next P2 piece was persistence. Chose **D1** over Vectorize ("more like this") because **D1 runs fully locally in `wrangler dev`** (so it's end-to-end verifiable now) whereas Vectorize has no real local emulation. Built: `src/store.js` + `migrations/0001_create_tracks.sql` — every generate/modify/render is logged as a `tracks` row (Contract 5: id/session_id/prompt/instruction/source/code/share_url/parent_id/version/created_at; lowercase snake_case per the D1 lessons), with the modify chain linked via `parent_id`. New **`GET /history?session_id=…&limit=…`** (bearer-gated, newest-first; scoped or global). Persistence is **best-effort** — a D1 failure is logged but never breaks the music response. And, per my own Cloudflare rule (*retention cron in the same PR as the table*), a **daily Cron Trigger** → `scheduled()` → `pruneTracks` deletes rows older than `RETENTION_DAYS`=30 so the log can't grow into D1's 10 GB cap. **Tested in real `workerd`** (`wrangler dev` + mock OpenAI, local D1, `--persist-to` isolated → hermetic across runs): `/history` scoped + global + empty + 401, and a **full retention E2E** — backdate a row via `d1 execute`, fire `/__scheduled`, assert the old row is pruned and recent rows survive. `worker/test.sh` now 22 unit + 27 integration checks; `scripts/test.sh` **10/10**; `wrangler deploy --dry-run` bundles DO+D1+cron clean (18.4 KiB). Reviewed via vibe-orchestrator. **Next:** Vectorize embeddings (P2 last piece) or P1 container render.
- **Jun 22 — Phase 3 P2 (modify slice): the modify loop — the demo's core differentiator — now runs on the edge Worker.** With P0 (the edge Worker) shipped, the next-highest-impact item was the **modify loop**: the roadmap repeatedly calls it "the core / the differentiator" (Situation E, demo step 3 — *edit the code, don't re-roll a black box*), and on the Worker `/generate` was stateless (no "faster"/"darker" follow-ups). Chosen over P1 (container audio — needs Docker + a paid account, can't verify in-session) because **Durable Objects run fully locally in `wrangler dev`**, so it's both higher-impact and end-to-end testable now. Built: a per-session **Durable Object** (`worker/src/session.js`, SQLite-backed) holding the ordered version chain + **`POST /modify {session_id, instruction}`** — loads the latest version, has `gpt-5.4` edit the code (same validate + auto-repair gate), stores the new version, returns the new code + `share_url` + a **code `diff`** + `version`/`parent_id`. `/generate` gained an optional `session_id` to start a chain. **Tested in real `workerd`** (`wrangler dev` + mock OpenAI): generate→v1, modify→v2 (parent 1, diff), modify→v3 chains on v2, unknown session→404, plus auth/validation. **Caught + fixed a test-hygiene bug:** wrangler dev persists DO state across runs (default `.wrangler/state`), so a second run drifted the version numbers — fixed by `--persist-to` a fresh temp dir so each run is hermetic (verified by running the suite twice → identical). `worker/test.sh` now 16 unit + 20 integration checks; `scripts/test.sh` **10/10 green**; `wrangler deploy --dry-run` bundles the DO clean (12.9 KiB). Reviewed via vibe-orchestrator. **Next (P2 rest):** `tracks` in D1 + embeddings in Vectorize.
- **Jun 22 — Phase 3 P0: ZeroClaw orchestrator now runs on a Cloudflare Worker (off the laptop — the user's #1 roadmap ask).** Of the two remaining high-leverage moves (Supabase creds = external/blocked; the Cloudflare migration = the marquee "get off localhost" win, entirely unbuilt), built the first thin vertical slice: **`worker/`**, a Worker that ports the `api-server.py` request/response surface to the edge — `POST /generate` (prompt → `gpt-5.4` via `fetch` → valid Strudel → `strudel.cc` link) + `POST /render` + `GET /health`/`/`. **Tier-A only** (`audio_url:null`; the headless render needs a Container = P1), so it's genuinely shippable now: always-on, stable URL, no laptop, retires the ephemeral quick tunnel. Faithful to the local system — bearer auth (`MUSIC_API_TOKEN`), the same auto-repair-on-parse-error loop, and a `share_url` base64 **proven byte-identical** to the Python/Node link builders (so edge links == laptop links). Follows the project's gpt-5.4 rules (flat `reasoning_effort`, `max_completion_tokens`, no `temperature`); `OPENAI_BASE_URL` is AI-Gateway-ready. **Tested:** `worker/test.sh` → 12 unit tests + a live `wrangler dev` (real `workerd`) integration pass against a **mock OpenAI** — full `/generate`→200 path plus health/CORS/auth(401)/validation(400/422), all deterministic & offline (no key, no cost); added to `scripts/test.sh` (**10/10 suites green**), and `wrangler deploy --dry-run` bundles clean. **Next (P1):** Worker → Queues → Container (Playwright+ffmpeg) → R2 so `/render` returns real audio.
- **Jun 22 — Faster-feeling song delivery: post section links FIRST + an expectation-setting note (the "old response" report).** A user reported a full-song reply as "old / not working" — but it *was* delivered: voice message at **+57 s**, section links at +57.7/+58.3 s. The audit found everything correct (code fully pushed — HEAD `ea6578b` == origin; **no stale/duplicate instances** — one launchd-managed process each; watcher current + healthy + cycling). The real issue was **latency**: a full ~2.7-min arrangement renders faithfully in ~50 s, so the text reply shows long before the audio. Two fixes so it stops *feeling* broken: **(1)** the watcher posts the **per-section links FIRST** (instant — no render), so the user gets playable links in ~10 s while the audio renders; the voice message follows. `already_delivered()` was refactored to recognize delivery via a voice msg **or** a section-links msg in either order, so idempotency/re-scan still holds. **(2)** the soul's full-song note now says *"section links post in seconds; the rendered audio follows in ~1 min,"* so the listener knows it's rendering. **Tested:** `test-watch-sections.py` → all green incl. 6 order-agnostic `already_delivered` cases; soul examples still parse. Deployed (watcher + daemon kickstarted).
- **Jun 22 — Radio: declick fades at segment boundaries (smoother stream).** Independent HLS segments were hard-cut, which clicks/pops at the join between differently-keyed/tempo'd patterns. Live HLS can't true-crossfade independent segments, so the clean alternative is a short ~0.08 s fade-in/out per segment that removes the boundary pop without a perceptible gap — added to the segment transcode alongside the true-peak limiter. **Tested:** `test-radio.sh` → all green (segments still valid audio); a decoded faded segment stays non-silent (waveform peak 255/255). (Reviewed via vibe-orchestrator.)
- **Jun 22 — Radio "now playing" display — the evolving/steered vibe is visible live.** `radio-compose.mjs` now prepends a `// meta: {index,bpm,key,mode,kit,steer}` line (a comment the gate *and* the faithful renderer ignore — verified, so existing gate-tests stay green); `radio.sh` writes it to `<outdir>/now.json` for each segment as it goes on air; and the player (`radio.html`) polls it every 5 s, showing e.g. `♪ now: #7 · 124 bpm · A minor · TR909 · steer: darker`. Makes the "genuinely generative + evolving + steerable" point land in a demo — the tempo/key/mood visibly drift and react to steers. **Tested:** `test-radio.sh` → all green incl. 3 new checks (valid meta JSON · radio.sh writes now.json · player polls it). (Reviewed via vibe-orchestrator.)
- **Jun 22 — Full post-incident verification: production-ready; remaining work is external/large.** After ~15 iterations (the watcher rewrites — DNS-resilience, idempotency, watch-all-channels, 403-cache — plus offline rendering, auto-repair on both surfaces, per-section song links, the generative radio with steering + time-seed, and the self-heal watchdog), ran the whole suite together to confirm nothing regressed: **`scripts/test.sh` → all 9 suites pass** and **`strudel-doctor.sh` → 22/22, demo-ready**. The bot is deployed and resilient end-to-end — delivers in every channel the bot is in, self-heals all 3 services. **The substantive roadmap is now complete or externally-blocked:** Tier-B → Supabase persistence needs the frontend team's **creds**; the Phase-3 **Cloudflare** migration is multi-day. Further small commits are hardening/polish, not impact — the high-leverage moves are external (provide Supabase creds, or green-light + widen-cadence for the Cloudflare lift). *No code change this checkpoint — the full-suite + doctor run is the verification.*
