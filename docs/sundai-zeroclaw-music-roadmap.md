...
- **P0 — ✅ Local-first prototype built (`scripts/radio.sh`, Jun 22).** Renders a sequence of Strudel
  ...
  Verified by `scripts/test-radio.sh` (valid HLS + real-audio segments). Listen:
  `(cd <out> && python3 -m http.server 8123)` → play `stream.m3u8` in ffplay/VLC/Safari. **P1 next:** the
  evolution engine (derive each next pattern from the last), agent-generated segments, and a rolling-window
  playlist (evict old segments + bump `EXT-X-MEDIA-SEQUENCE`).
- **P1 — Evolution engine. ✅ Core built (`scripts/radio-compose.mjs`, Jun 22).** A deterministic
  ...
  `radio.sh` now sources each segment from it instead of a fixed seed cycle. **Still P1:** derive each
  segment from the *previous* one (a true transition model) · agent-generated segments · seamless
  harmonic-mix **crossfades** at joins (ffmpeg `acrossfade`) · a rolling-window playlist.
